...
void dlo::OdomNode::getNextPose() {
  ...
  //Eigen::Matrix4f T_S2S = this->imu_SE3;
  // Eigen::Matrix3f  RotationMatrix = T_S2S.block(0,0,2,2);
  // Eigen::Vector3f  eulerAngle = RotationMatrix.eulerAngles(2,1,0);
  // Eigen::AngleAxisf rollAngle(eulerAngle(2),  Eigen::Vector3f::UnitX());
  // Eigen::AngleAxisf pitchAngle(0,  Eigen::Vector3f::UnitY());
  // Eigen::AngleAxisf yawAngle(eulerAngle(0),  Eigen::Vector3f::UnitZ()); 
  // RotationMatrix=yawAngle*pitchAngle*rollAngle;
  // T_S2S.block(0,0,2,2) = RotationMatrix ;
      /**
     *@author Goldmann
     *@date 2023-1-30
     *@brief z0
...
//  Eigen::Matrix3f RotationMatrix = T.block(0,0,2,2);
// Eigen::Vector3f eulerAngle = RotationMatrix.eulerAngles(2,1,0);
//   Eigen::AngleAxisf rollAngle1(eulerAngle(2),  Eigen::Vector3f::UnitX());
//   Eigen::AngleAxisf pitchAngle1(0,  Eigen::Vector3f::UnitY());
//   Eigen::AngleAxisf yawAngle1(eulerAngle(0),  Eigen::Vector3f::UnitZ()); 
//   RotationMatrix=yawAngle1*pitchAngle1*rollAngle1;
//   T.block(0,0,2,2) = RotationMatrix ;
...
//    Eigen::Matrix3f  RotationMatrix = this->rotq.matrix();
//   Eigen::Vector3f  eulerAngle = RotationMatrix.eulerAngles(2,1,0);
//   Eigen::AngleAxisf rollAngle(eulerAngle(2),  Eigen::Vector3f::UnitX());
//   Eigen::AngleAxisf pitchAngle(0,  Eigen::Vector3f::UnitY());
//   Eigen::AngleAxisf yawAngle(eulerAngle(0),  Eigen::Vector3f::UnitZ()); 
//   RotationMatrix=yawAngle*pitchAngle*rollAngle;
//   this->rotq = RotationMatrix ;
}
...
void dlo::OdomNode::debug() {
  ...
  std::cout << std::endl << std::right << std::setprecision(2) << std::fixed;
  std::cout << "T[4x4] :: " << T(0,0) << " " <<T(0,1) << " " << T(0,2) << " " << T(0,3)<< std::endl;
  std::cout << "T [4x4] :: " << T(1,0) << " " <<T(1,1) << " " << T(1,2) << " " << T(1,3)<< std::endl;
  std::cout << "T [4x4] :: " << T(2,0) << " " <<T(2,1) << " " << T(2,2) << " " << T(2,3)<< std::endl;
  std::cout << "T [4x4] :: " << T(3,0) << " " <<T(3,1) << " " << T(3,2) << " " << T(3,3)<< std::endl;
  std::cout << std::endl << std::right << std::setprecision(2) << std::fixed;
  std::cout << "T_s2s_prev[4x4] :: " << T_s2s_prev(0,0) << " " <<T_s2s_prev(0,1) << " " << T_s2s_prev(0,2) << " " << T_s2s_prev(0,3)<< std::endl;
  std::cout << "T_s2s_prev [4x4] :: " << T_s2s_prev(1,0) << " " <<T_s2s_prev(1,1) << " " << T_s2s_prev(1,2) << " " << T_s2s_prev(1,3)<< std::endl;
  std::cout << "T_s2s_prev[4x4] :: " << T_s2s_prev(2,0) << " " <<T_s2s_prev(2,1) << " " << T_s2s_prev(2,2) << " " << T_s2s_prev(2,3)<< std::endl;
  std::cout << "T_s2s_prev [4x4] :: " << T_s2s_prev(3,0) << " " <<T_s2s_prev(3,1) << " " << T_s2s_prev(3,2) << " " << T_s2s_prev(3,3)<< std::endl;


    std::cout << std::endl << std::right << std::setprecision(2) << std::fixed;
  std::cout << "T_S2S [4x4] :: " << T_s2s(0,0) << " " <<T_s2s(0,1) << " " << T_s2s(0,2) << " " << T_s2s(0,3)<< std::endl;
  std::cout << "T_s2s [4x4] :: " << T_s2s(1,0) << " " <<T_s2s(1,1) << " " << T_s2s(1,2) << " " << T_s2s(1,3)<< std::endl;
  std::cout << "T_s2s [4x4] :: " << T_s2s(2,0) << " " <<T_s2s(2,1) << " " << T_s2s(2,2) << " " << T_s2s(2,3)<< std::endl;
  std::cout << "T_s2s [4x4] :: " << T_s2s(3,0) << " " <<T_s2s(3,1) << " " << T_s2s(3,2) << " " << T_s2s(3,3)<< std::endl;
