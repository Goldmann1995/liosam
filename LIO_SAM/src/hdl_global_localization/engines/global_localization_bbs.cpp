...
void GlobalLocalizationBBS::set_global_map(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud) {
  BBSParams params;
  params.max_range = private_nh.param<double>("bbs/max_range", 15);
  params.min_tx = private_nh.param<double>("bbs/min_tx", -2.0);
  params.max_tx = private_nh.param<double>("bbs/max_tx", 2.0);
  params.min_ty = private_nh.param<double>("bbs/min_ty", -2.0);
  params.max_ty = private_nh.param<double>("bbs/max_ty", 2.0);
  ...
  double map_max_z = private_nh.param<double>("bbs/map_max_z",2);
  ...
  double map_resolution = private_nh.param<double>("bbs/map_resolution", 1);
...
GlobalLocalizationResults GlobalLocalizationBBS::query(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, int max_num_candidates) {
  double scan_min_z = private_nh.param<double>("bbs/scan_min_z", -2);
  double scan_max_z = private_nh.param<double>("bbs/scan_max_z", 2.4);
